...
These files together form the FM-index (Ferragina-Manzini index), a compressed representation of the reference genomes that enables efficient sequence alignment.

- `INDEX.1.bt2`, `INDEX.2.bt2`, `INDEX.3.bt2`, `INDEX.4.bt2` – These are the forward strand index files generated by bowtie2-build. Bowtie2 uses these files to match sequencing reads to locations on the forward strand of the reference genomes.

- `INDEX.rev.1.bt2`, `INDEX.rev.2.bt2` – These are the reverse-complement index files, used for aligning reads that map to the reverse strand of the reference genomes.
...
1. Mapping paired-end reads to the set of representative MAGs (reference genomes) using bowtie2.
...
3. Filtering alignments using msamtools, retaining only reads that:
  ...
  - Have at least 95% identity with the MAG reference (-p 95).
